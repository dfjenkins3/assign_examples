...
#Input Files
################################################################################
#Input matrix with all test and training data
rpkm_file <- "merged.rpkmlog"
#Input phenotype text file with batch information and sample names
...
#Read in input data
nonzerolog <-read.table(rpkm_file, header=TRUE, row.names=1)

#Read in pheno.txt file that contains phenotype information, example available
#in example_files/pheno.txt
pheno <- read.table(pheno_file, header=TRUE, row.names=1)

#Create a model matrix for ComBat, just an intercept term because we are just
#adjusting for batch
modcombat = model.matrix(~1, data=pheno)

#Batch information from pheno.txt file
batch <- pheno$batch

#Perform ComBat
combat_rpkmlog <- ComBat(dat=nonzerolog, batch=batch,
                         mod=modcombat,  numCovs=NULL)
...
#Create a subset of the matrix containing only the training data (modify to
#match your samples)
...
#Create a subset of the matrix containing only the test data (modify to match
#your samples)
