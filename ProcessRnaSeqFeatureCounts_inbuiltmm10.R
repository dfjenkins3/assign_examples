...
if (length(command_args) != 5){
    ...
    print("Rscript <reference_path> <in_1.fq.gz> <in_2.fq.gz> <out_prefix> <threads>")
...
outFilePrefix = command_args[4]
nthreads = command_args[5]
...
#Create fpkm list
fCountsList = featureCounts(outBamFilePath, annot.inbuilt="mm10", nthreads=nthreads, isPairedEnd=!is.null(inFilePath2))
